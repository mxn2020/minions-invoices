---
name: minions-invoices
id: OC-0100
version: 1.0.0
description: "Agent skills for working with Minions Invoices MinionTypes. Provides CRUD operations, CLI usage, and best practices for AI agents managing minions-invoices data."
category: ai
subcategory: general
...
# Minions Invoices Agent Skills

Skills for agents operating on the `minions-invoices` toolbox.
...
```bash
# TypeScript
pnpm add @minions-invoices/sdk

# Python
pip install minions-invoices

# CLI
pnpm add -g @minions-invoices/cli
```
...
The `invoices` CLI provides basic project info and utilities:
...
```ts
import { customTypes } from '@minions-invoices/sdk/schemas';
...
```python
from minions_invoices.schemas import custom_types
...
- This agent only writes to `minions-invoices` — it reads from other toolboxes but never writes to them